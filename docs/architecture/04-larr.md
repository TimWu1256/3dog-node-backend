...
| `GenAIDataChannelManager` | 監聽 `CreatePeer.OnPeerCreated`，自動在每個新 peer 的 PeerConnection 上掛載 `GenAIDataChannel`；提供 `Broadcast()` 廣播與集中接收（`OnMessageReceived(senderId, message)`）|
...
| `GenAIDataChannel` | 獨立於 LARR 的應用層 DataChannel（ID 7，label "GenAI"）；提供 `Send()` 與 `OnMessageReceived` 事件 |
...
| 腳本 | 存在於 | 說明 |
|------|--------|------|
| `GenAIDataChannel` | Client + Server | 單一 PeerConnection 的 DataChannel 封裝；`Setup()` 供 Manager 程式化初始化 |
| `GenAIDataChannelManager` | Server | Singleton；自動為每個 peer 建立 `GenAIDataChannel`，提供廣播與集中接收；`ConnectedPeerCount` 屬性供 Capture 流程查詢當前連線數 |
...
```
任意 Client 送出訊息
  → GenAIDataChannel.OnMessageReceived
  → GenAIDataChannelManager.OnMessageReceived(senderId, message)  ← 集中處理點

Server 廣播
  → GenAIDataChannelManager.Broadcast(message)
  → 所有已連線 Client 的 GenAIDataChannel.Send()
```
...
所有在 GenAI DataChannel（ID 7）上傳輸的訊息均為 **UTF-8 JSON**，必須包含 `type` 字串欄位。接收端根據 `type` 值分派至對應的處理邏輯。所有 `type` 常數定義於 `GenAIDataChannelProtocol.cs`。
...
[Server] CapturePhotoHandler.StartCapture()
  ...
  ├─ 訂閱 GenAIDataChannelManager.OnMessageReceived
