...
- **Pre-injected globals:** `THREE` (Three.js r182) and `GLTFExporter` are already available as function parameters.
...
1. (`THREE` and `GLTFExporter` are already in scope.)
...
**ENSURE THE CODE IS VALID JAVASCRIPT AND ENDS BY STRICTLY CALLING `__export(scene)` (or your root `THREE.Object3D`).**
